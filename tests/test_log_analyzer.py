...
class TestLogLocation(unittest.TestCase):
    ...
    def test_latest_gz_file(self):
        ...
        self.assertEqual(found_file.filename, 'nginx-access-ui.log-20160630.gz')

    def test_bz_log_file(self):
        found_file = log_analyzer.find_latest_log(test_config.get('LOG_DIR'))
        self.assertNotEqual(found_file.filename, 'nginx-access-ui.log-20180630.bz')

    def test_invalid_log_file(self):
        found_file = log_analyzer.find_latest_log(test_config.get('LOG_DIR'))
        self.assertNotEqual(found_file.filename, 'packet_capture.log-20180630.gz')

    def test_plain_log_file(self):
        found_file = log_analyzer.find_latest_log(os.path.join(test_config.get('LOG_DIR'), "./plain_dir"))
        self.assertEqual(found_file.filename, 'nginx-access-ui.log-20160630')
...
class TestLogAnalyzing(unittest.TestCase):
    ...
    def test_file_analyzed(self):
        format_data = "%Y%m%d"
        new_date = datetime.strptime('20160630', format_data)
        analyzed_file = log_analyzer.is_file_analyzed(new_date, config=test_config)
        self.assertEqual(analyzed_file, True)

    def test_empty_log(self):
        self.assertEqual(log_analyzer.handle_file(log_analyzer.read_file_into_data,
                                                  log_file="nginx-access-ui.log-20160630.gz",
                                                  config=test_config),
                         None)

    def test_many_errors(self):
        test_file = os.path.join('plain_dir', 'nginx-access-ui.log-20160630')
        with self.assertRaises(ValueError):
            test = log_analyzer.handle_file(log_analyzer.read_file_into_data, log_file=test_file, config=test_config)
    ...
    def test_result_generator(self):
        ...
        test = log_analyzer.handle_file(log_analyzer.read_file_into_data,
                                        "nginx-access-ui.log-20180630", config=new_config)
...
class TestReportCreation(unittest.TestCase):
    report = os.path.join(test_config.get("REPORT_DIR"), "report-2018.06.30.html")
